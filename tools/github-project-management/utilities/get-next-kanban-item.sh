...
if [ "${1:-}" = "--auto-assign" ]; then
    ...
    if [ -n "$ITEM_ID" ] && [ "$ITEM_ID" != "null" ]; then
        # Use the update-task-status-simple.sh script if it exists
        if [ -f "./tools/github-project-management/utilities/update-task-status-simple.sh" ]; then
            ./tools/github-project-management/utilities/update-task-status-simple.sh "$ITEM_NUMBER" "In Progress"
        else
            echo -e "${YELLOW}Warning: Status update script not found, skipping status update${NC}"
        fi
    else
