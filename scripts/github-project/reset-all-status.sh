...
# Script to reset status field for ALL issues (epics, stories, tasks) to Todo

OWNER="o2alexanderfedin"
REPO="telethon-architecture-docs"
PROJECT_NUMBER=12

echo "🔄 Resetting Status Field for ALL Issues..."
echo "=========================================="
echo ""
...
update_item_status() {
    ...
    if [ -n "$STATUS_FIELD_ID" ] && [ -n "$TODO_ID" ]; then
        gh api graphql -H "GraphQL-Features: project_v2" \
          ...
          -F value="$TODO_ID" > /dev/null 2>&1
        
        echo "  ✅ #$issue_number ($issue_type) - Status reset to Todo"
        return 0
    fi
    return 1
}

echo "📝 Resetting status for all project items..."
echo "==========================================="
echo ""
...
while [ "$HAS_NEXT_PAGE" = "true" ]; do
    ...
    echo "$ITEMS_DATA" | jq -c '.data.user.projectV2.items.nodes[]' | while read -r item; do
        ...
        # Update status
        if update_item_status "$ITEM_ID" "$ISSUE_NUMBER" "$ISSUE_TYPE"; then
            ((UPDATED_COUNT++))
...
echo "Items updated: $UPDATED_COUNT"
echo ""
echo "By Type:"
...
echo ""
echo "All issues have been reset to 'Todo' status!"